        ...
        for fld_idx in 1:length(si.defs)
            ...
            if !@isdefined(slot_id)
                ...
                if haskey(vars, fld_idx)
                    ...
                    has_def && maybe_add_allocopt_todo(ctx, fld_def)
                    continue
                end
            end
            ...
            if has_def
                ...
                ctx.sv.src.slottypes[slot_id] =
                    Union{slot_type,widenconst(exprtype(fld_def, ctx.sv.src, ctx.sv.mod))}
            end
        end
        def = info.defs[i]
        ctx.changes[def.stmts=>def.stmtidx] = nothing
